---
name: docs-style
description: The source-documentation standard for this repository — how file headers, exported-symbol blocks, inline comments and cross-references are written. Load before any documentation pass, before writing or revising a module header or TSDoc block, and when reviewing comments for accuracy. Covers the governing "conclusion, not investigation" rule, voice, header shape, depth by code kind, `{@link}` resolution rules, and the six questions to answer before documenting an existing file.
---

# Documentation standard

The rules below govern every comment written in this repository. `npm run docs:links` enforces the
cross-reference half; nothing enforces the rest, so it is read before a pass rather than after one.
