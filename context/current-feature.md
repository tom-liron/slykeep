...
- **Dashboard UI Phase 2** (`feature/dashboard-phase-2`) — Built the functional collapsible sidebar: item types linking to `/items/[slug]` (colored icons + counts), favorite and recent collections sections, and a user avatar area with settings link. Moved the DevStash brand out of the sidebar into a full-width top bar so it stays visible when the rail is collapsed and on mobile. Added a mobile slide-out drawer (hamburger to open, X/backdrop/Escape to close, body scroll lock) and made the top bar responsive across all breakpoints.
