...
`updateItem(itemId, data)` in `src/actions/items.ts` following the `{ success, data, error }` return pattern. Validates input with Zod, resolves the user via `getCurrentUserId()` from `@/server/current-user`, validates ownership, and performs the write.

## Data

- The write lives in the action itself, calling `prisma` directly — the same shape as the existing
  writes in `src/actions/account.ts`. `src/server/items.ts` stays read-only, per
  `context/project-overview.md` §9.
- Tag handling on update: disconnect all existing tags, connect-or-create new ones
- Returns the updated `ItemDetailViewModel` so the drawer can refresh without a second fetch
