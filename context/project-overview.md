...
- **A domain, and `EMAIL_FROM` on it.** Decided, not open: everything email-shaped waits for a real
  domain and is fixed together at that point. Until then `EMAIL_FROM` is unset, so `lib/email.ts`
...
- **An unverified account holds its email address forever.** Registration creates the `User` row before the verification email is sent, and nothing ever removes it: `emailVerified` stays null, the account can never sign in (`authorize` refuses it), and the address is permanently taken. A typo at signup — `tomm@` for `tom@` — therefore locks that address out of the product for good, and the person it actually belongs to hits the 409 in `api/auth/register` with no route forward, since they cannot verify an account they did not create. The 24-hour token expiry does not help: the token dies, the row does not. Not a security hole — the honest `emailVerified: null` is what keeps the account-linking work above safe — but a permanent denial of service on one address per typo. OWASP is silent on it (the Email Validation cheat sheet covers format and token lifetime only), so the reference points are conventions: **GitLab deletes unconfirmed accounts after 3 days**, and its own issue proposing a configurable window suggests 7 as the default, naming this exact mistyped-address case as the motivation. Two fixes, which solve different halves. A **scheduled sweep** deleting `emailVerified: null` rows older than a week is the one worth building: it changes no response, risks nothing (an unverified account owns no items or collections, so there is nothing to cascade), and fits the existing `scripts/` shape with Vercel Cron to run it. **Re-registration takeover** — letting a fresh signup replace an unverified row with a new hash and a new token — is what a sweep cannot do, because "come back in a week" is no answer to someone who spotted the typo immediately; it is safe, since only the address owner receives the link and a squatter's password is discarded, at the cost of answering 201 where the route now answers 409, which widens an existence disclosure this route already makes deliberately. Deferred: with no real users a squatted address costs nothing, and the sweep is the half to build first.
