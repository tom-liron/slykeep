...
/**
 * The item write path: create, edit, favourite, pin and delete.
 *
 * The boundary between the item UI and Prisma, and the busiest write module in the application.
 * `CreateItemDialog`, `ItemEditForm` and the drawer's toolbar call these actions, which authenticate
 * the caller, validate the payload against `lib/item-schemas.ts`, enforce the free-tier caps and the
 * Pro type gate, and perform the write. `server/items.ts` stays read-only — `server/` owns reads and
 * `actions/` owns writes.
 *
 * Three things beyond Postgres hang off these writes: an item's R2 object is deleted with it, the
 * collections an item touches have their recency moved by {@link touchCollections}, and the
 * `contentType` invariant — that a snippet is TEXT and a link is URL — is enforced here rather than
 * by the schema.
 *
 * @remarks
 * Every payload field that names something the caller could have tampered with is re-derived or
 * re-checked here: `userId` from the session, the item type's id from the catalog name, its
 * `contentType` from the catalog, the R2 key from `isOwnedKey`, and the collection ids from
 * {@link ownsEveryCollection}. Ownership always lives in the `where`, so another user's id is
 * refused by the same path as one that does not exist.
 */

/**
 * Moves `updatedAt` on the collections an item write just affected, which is what makes "recent
 * collections" mean recent *activity*.
 *
 * Every recent-collections listing orders by that column, but membership lives in `item_collections`
 * and is written as a nested write on the **Item** — so Postgres never touches the `collections`
 * row, and without this `updatedAt` equals `createdAt` for every collection nobody has renamed.
 *
 * @remarks
 * A write rather than a read-time aggregate: deriving recency from the greatest of the collection's
 * own timestamp, its items' and `ItemCollection.addedAt` would put a joined aggregate no index can
 * serve into the `ORDER BY` of two queries that run on every dashboard page view, to save one
 * `UPDATE` on a path that is already writing.
 *
 * Scoped to the owner even though every caller has already established ownership — ids reaching a
 * bulk `updateMany` are worth pinning to the account at the statement itself.
 *
 * Best-effort: the item write it follows has already succeeded and been reported, so a failure here
 * costs a sidebar ordering rather than the user's work.
 */
 ...
 *
 * @remarks
 * The ids reach the server the way `fileKey` does — chosen from a list this account was served, then
 * posted back — so by the time they arrive they are client input again, and a hand-made payload
 * could name a collection belonging to someone else. Nothing else on the write path would catch it:
 * `ItemCollection` has no `userId` of its own, so the insert would file this user's item into a
 * stranger's collection and the stranger's collection page would render it.
 *
 * Counting is enough and is one query: the ids are already de-duplicated by the schema, so a count
 * short of the list means at least one is not theirs — or does not exist, which is the same answer,
 ...
 *
 * @remarks
 * Four things the payload is not trusted with. `userId` comes from the session, so an item can only
 * be created for the signed-in user. The item type's id is looked up from the submitted *name* —
 * never accepted directly — with `findFirst({ name, userId: null })` rather than `findUnique`, since
 * `name` is unique only among system rows. `contentType` is read from the catalog, so it always
 * agrees with the type. And the R2 key is re-derived: `POST /api/upload` hands one to the browser
 * and the browser hands it back here, so `isOwnedKey` establishes who it belongs to from the session
 * rather than taking the round trip's word for it.
 */
export async function createItem(input: CreateItemInput): Promise<CreateItemResult> {
    ...
    // the UI may *show* the cap, but the action is the authority. A hard block rather than a nag,
    // because the pricing page promises "Up to 50 items" and a 51st that succeeds turns the number
    // into decoration.
    //
    // An accepted race: two concurrent creates can both read 49 and both write. Closing it needs a
    // transaction with a re-count on the hottest write path in the application, which is not worth
    // it for a soft cap.
    ...
    if (columns.fileKey) {
        ...
        // what the `.svg` exception exists to prevent, and which `nosniff` cannot help with, because
        // the declared type is the truth about those bytes.
        //
    ...
    try {
        ...
        if (tags?.length) {
            // The same "ensure these exist" as `updateItem`. Tags are per-account, so a name another
 ...
 *
 * @returns The whole updated item on success, re-read rather than assembled from the payload: the
 * drawer re-renders its body from what comes back, and it renders the item's collections and
 * database-set timestamps, which this action does not compute.
 *
 * @remarks
 ...
 * the item's type, because an edit payload does not carry one — the type is not editable.
 */
export async function updateItem(
    ...
    try {
        // The item's type says which content columns it has, and an edit payload never carries one,
        // so it is read here rather than inferred from which fields arrived. `createItemSchema` does
        // the same stripping in a `.transform()`, which it can do only because the create dialog
        // submits the type; this read is the equivalent step for the path that does not.
        //
        ...
        if (!isItemTypeName(existing.itemType.name)) {
            ...
            // it can: nothing here knows what a custom type owns, and the alternative is silently
            // dropping an edit to a column that may well exist.
        ...
        await prisma.item.update({
            ...
            // rule `getItemDetail` follows. Kept here as well as on the read above, so the
            // authorization does not depend on that read having happened.
            where: { id: itemId, userId },
            data: {
                title,
                description,
                ...
                // payload cannot put a URL on a snippet and make `contentType` a lie. Every field is
                // named rather than spread, so a column added to the schema has to be considered
                ...
                // One of exactly two writes to `editedAt` — the other is `createItem`. The column is
                // stamped by hand, rather than declared `@updatedAt` like the row's other timestamp,
                // so that it moves only when content changes: the favourite and pin toggles write a
                // boolean to the same row, and an automatic column would treat that as an edit and
                // reshuffle every recency listing. `updatedAt` still moves on all of them.
                editedAt: new Date(),
                ...
                // re-inserting: the rows that survive keep their `addedAt`, which is the only record
                // of when an item was filed somewhere.
                //
                // The two halves work on disjoint sets — one deletes what is no longer selected, the
                // other inserts what is newly selected and skips what is already there — so they hold
                // whichever order Prisma runs them in. `undefined` leaves the relation alone, exactly
                // as it does for tags.
        ...
        // The union of where the item was and where it now is. Both halves matter, for different
        ...
        // the item is still activity for every collection holding it — which is the common case,
        // since most edits are to content rather than to filing.
...
/**
 * Favourites or unfavourites an item, from the drawer's toolbar or a card's star.
 *
 * @param isFavorite - The state to write, for the reasons set out on `toggleCollectionFavorite`: the
 * write is idempotent, needs no read in front of it, and cannot be raced into disagreeing with the
 * star that triggered it.
 *
 * @remarks
 * No re-read, unlike `updateItem`: this touches one boolean the caller is already rendering.
 * `updatedAt` still moves via `@updatedAt`, which is what surfaces a just-favourited item at the top
 * of `/favorites`.
 *
 * And no `revalidatePath`, where `toggleCollectionFavorite` has one. That one writes to a list the
 * dashboard *layout* renders; nothing in the layout renders an item's star, so refreshing the route
 * the drawer is open over is enough.
 */
...
/**
 * Pins or unpins an item, from the drawer's toolbar.
 *
 * The same write as {@link toggleItemFavorite} on a different column. The two stay separate actions
 * rather than one parameterised toggle: the column would then be a string from the client, which is
 * a wider door than two boolean writes.
 *
 * @remarks
 * This write moves the row's automatic `updatedAt` and leaves `editedAt` alone, since pinning is not
 * editing. That is what keeps it out of every recency ordering in the application, all of which sort
 * by `editedAt`.
 *
 * `pinnedAt` is written in the same statement as the boolean and cleared to `null` on unpin. Two
 * columns rather than one nullable timestamp doing both jobs, so `isPinned` stays the thing every
 * listing filters and sorts on without reasoning about NULLs — but they are only ever consistent
 * because this one write sets both. It also means an unpin genuinely forgets when the item was
 * pinned: re-pinning puts it at the top of the section rather than back where it sat.
 */
...
/**
 * Deletes an item, its membership rows, and the R2 object behind it.
 *
 * A hard delete: there is no `deletedAt` column and no trash view, which is why the dialog in front
 * of this says so.
 *
 * One `delete` covers the whole row in Postgres. `ItemCollection` declares `onDelete: Cascade` on
 * its item side and the implicit `ItemTags` join table cascades the same way, so the join rows go
 * with the item without a transaction. `Tag` rows themselves survive: a tag belongs to the account
 * rather than to the item, so the user keeps their vocabulary for the next item and the rows go only
 * when the account does.
 *
 * @remarks
 * The object is deleted *after* the row. There is no transaction spanning Postgres and R2, so one
 * failure mode has to be chosen: delete the object first and a failed row delete leaves an item
 * pointing at nothing, which the drawer and the preview both surface to the user; delete it second
 * and a failed object delete leaves an orphan nobody can see. The orphan is the cheaper mistake —
 * logged, never raised.
 */
export async function deleteItem(itemId: string): Promise<DeleteItemResult> {
    ...
    try {
        ...
        // Ownership in the `where` for the reason `updateItem` puts it there: another user's id is
        // rejected by the same path as one that does not exist, so neither confirms the other. The
        // deleted row comes back, which is where the key to clean up comes from — reading it
    ...
    if (fileKey) {
        try {
            ...
            // would misdescribe what happened, and there is nothing they could do about it.
