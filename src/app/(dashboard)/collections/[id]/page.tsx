...
/**
 * A single collection at `/collections/<id>`: its header — name, favourite star, description, and a
 * per-type item-count breakdown — above a paginated grid of the items in it.
 *
 * `getCollectionPageData` scopes the read to the user and returns `null` for an id that is not
 * theirs, which becomes a 404. `CollectionActions` here takes `afterDeleteHref="/collections"`,
 * since this route 404s the moment the row is gone.
 */
    ...
    return (
        <div className="mx-auto max-w-6xl space-y-6">
            <header className="space-y-2">
                {/* The actions stack below the title on a phone. Star, Edit and Delete need ~200px
                    and are `shrink-0`, so inline on a narrow row they force the collection name to
                    truncate — on the one page whose subject is that name. */}
